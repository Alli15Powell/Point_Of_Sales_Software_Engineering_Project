...
public class SecondaryController implements Initializable {
    ...
    FXMLLoader loader = App.setRoot("tertiary");
    TertiaryController controller = loader.getController();
    controller.setFinalTotal(total);
    createReciept();
    }
    ...
    private void priceChange(){
        ...
        //This has to be here, broke when I try to implement it elsewhere
    public void createReciept() throws IOException{
        int receiptCharacter = 0;
        //Generates the reciept ID. I thought of simply having it generate a 15 digit long number but I figured it would be better if it just generated 15 values and appended it together.
        StringBuilder stringRecieipt = new StringBuilder();
        int receiptIDLength = 15;
        for (int i = 0; i < receiptIDLength;i++){
            receiptCharacter = ThreadLocalRandom.current().nextInt(10);
            stringRecieipt.append(receiptCharacter);
            
        }

        String fileName = "pos-system\\src\\main\\Receipts\\Receipt Number " + stringRecieipt.toString() + ".txt";
        receiptID = "pos-system\\src\\main\\Receipts\\Receipt Number " + stringRecieipt.toString() + ".txt";
        //Thing that actually writes to a file
        PrintWriter outFile = new PrintWriter(fileName, "UTF-8");
        outFile.println("Receipt ID: " + stringRecieipt);
        outFile.println("THANK YOU FOR SHOPPING");
        outFile.println("Items");
        outFile.println("---------------------------------------------------------");
        for (CartSystem item : CartDisplay.getItems()) {
            outFile.println(item.getItem() + " . . . . . . . . . " + item.getPrice());
        }
        outFile.println("---------------------------------------------------------");
        outFile.println("Subtotal: $" + String.format("%.2f", subtotal));
        outFile.println("Discounts: $" + String.format("%.2f", displayDiscount));
        outFile.println("Tax: $" + String.format("%.2f", tax));
        outFile.println("---------------------------------------------------------");
        outFile.println("Total: $" + String.format("%.2f", total));
        outFile.close();
